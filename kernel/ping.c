...
void run_real_ping() {
    ...
    ping.type = 8; ping.code = 0; ping.id = 0x1337; ping.seq = 1;
    for(int i=0; i<32; i++) ping.payload[i] = 'A';
    ...
    if(received) {
        ...
        printk("\033[32m[PING]\033[0m Reply desde 10.0.2.2: time=\033[36m%d ms\033[0m\n", (uint32_t)ms);
    } else {
