...
void *pmm_alloc_frame() {
  ...
  for (uint32_t i = mm.next_free; i < mm.total_pages; i++) {
    ...
    /* Check if the bit is 0 (Free) and not protected */
    if (!(mm.bitmap[byte] & (1 << bit)) &&
        !(mm.protection_bitmap[byte] & (1 << bit))) {
      ...
      /* Address = Page Index * Page Size + base physical offset */
      uintptr_t paddr = (uintptr_t)i * PAGE_SIZE + mm.phys_limit_start;

      /* Optional: Log the allocation for debugging */
      // boot_msg("PMM: Allocated frame at 0x%p\n", paddr);

      return (void *)paddr;
    }
  }

  /* 5. If no frame was found from next_free, wrap around and search from the
   * start */
  ...
  for (uint32_t i = 0; i < mm.next_free; i++) {
    ...
    if (!(mm.bitmap[byte] & (1 << bit)) &&
      ...
      mm.bitmap[byte] |= (1 << bit);
      mm.next_free = i + 1;
      return (void *)((uintptr_t)i * PAGE_SIZE + mm.phys_limit_start);
    }
  }

  /* 6. Out of Physical Memory (OOM) */
  boot_msg("PMM", "System is out of physical memory!\n", 2);
  return NULL;
}

void pmm_free_frame(void *paddr) {
  uintptr_t phys = (uintptr_t)paddr;
  if (phys < mm.phys_limit_start)
    return;

  uint32_t frame_index = (phys - mm.phys_limit_start) / PAGE_SIZE;
