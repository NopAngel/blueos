...
void vmm_init() {
  ...
  for (uint32_t addr = 0x80000000; addr < (uint32_t)_end + (1024 * 1024);
       addr += PAGE_SIZE) {
...
void map_page(page_table_t *root, vaddr_t vaddr, paddr_t paddr,
              uint32_t flags) {
  ...
  page_table_t *leaf_table =
      (page_table_t *)(((root->entries[vpn1] >> 10) << 12));
...
void *vmm_map_io(uintptr_t phys_addr, size_t size) {
  ...
  /*
   * map_page(root_page_table, virt_addr, phys_addr, PAGE_READ | PAGE_WRITE |
   * PAGE_NOCACHE);
   */
...
int vmm_map_page(uintptr_t vaddr, uintptr_t paddr, uint32_t flags) {
  ...
  /* Get the current Page Directory (this is usually a global pointer in your
   * VMM) */
