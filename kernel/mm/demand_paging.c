 ...
 * @addr: The virtual address that caused the exception (CR2 on x86, stval on
 * RISC-V).
 *
...
int handle_demand_paging(struct task_struct *task, uintptr_t addr) {
  ...
  /* Virtual addresses can be anywhere, but we must map the START of the page
   * (4KB aligned) */
  uintptr_t aligned_addr = addr & ~(PAGE_SIZE - 1);

  /* 2. Security Check (Basic VMA validation) */
  /* In a real kernel, you'd check task->vma_list to see if this address is
   * 'owned' */
  if (addr < 0x1000) {
    boot_msg("Segmentation Fault: Access to NULL or reserved area at 0x%p\n",
             addr);
    return 0;
  }
  ...
  if (!phys_frame) {
    boot_msg("Out of memory: Swap or OOM-Killer needed for %s\n", task->name);
    return 0;
  }
  ...
  boot_msg("[MMU] Demand Paging: Mapped 0x%p -> 0x%p for %s\n", aligned_addr,
           (uintptr_t)phys_frame, task->name);
