#include <fs/ext2.h>
#include <blueos/io.h> // Necesitas inb, outb, inw
#include <lib/string.h> // Para memcpy y strcmp
...
// --- MOTOR DE DISCO (HARDWARE DIRECTO) ---
...
void ext2_init() {
    block_size = 1024; // Default inicial para leer SB
...
void ext2_read_file(uint32_t inode_num) {
    ...
    // Leemos solo el primer bloque de datos por ahora
    disk_read_block(inode->block[0], data);
