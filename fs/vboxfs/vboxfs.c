...
void vbox_send_request(void *request) { outl(vbox_ioport, (uint32_t)request); }
int vboxfs_mount(const char *folder_name) {
  printk("VBoxFS: Attempting to mount folder: %s...\n", folder_name);
...
void vboxfs_init(uint32_t ioport) {
  ...
  if (req.rc >= 0) {
    printk("VBoxFS: Communication established.\n");
  }
}
