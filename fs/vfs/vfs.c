...
static int root_mkdir(struct vfs_node *node, const char *name, uint16_t mode) {
  ...
  // FIXED: Forzamos la tabla de operaciones nativa de memoria
  child->ops = &root_ops; 
  child->parent = node;
...
static int root_create(struct vfs_node *node, const char *name, const char *content) {
  ...
  // FIXED: Aseguramos que use las operaciones de la raíz (RAM/RootFS)
  child->ops = &root_ops; 
  child->parent = node;
...
int vfs_write(vfs_node_t *node, const void *buffer, uint32_t size,
              uint32_t offset) {
...
int vfs_readdir(vfs_node_t *node, uint32_t index, struct vfs_dirent *dirent) {
  if (node && (node->type == VFS_TYPE_DIR) && node->ops && node->ops->readdir) {
    // We cast the function pointer call to handle the dirent as a generic
    // pointer
    int (*readdir_func)(struct vfs_node *, uint32_t, void *) =
        (void *)node->ops->readdir;
...
void vfs_init(void) {
  ...
  vfs_root->type = VFS_TYPE_DIR; /* Tu tipo formal de directorio de BlueOS */
  vfs_root->ops = &root_ops;

  vfs_current = vfs_root;

  /* Sincronizamos el alias global para los drivers externos */
...
int vfs_mount(const char *dev, const char *target) {
  ...
  for (int i = 0; i < (int)(sizeof(system_dirs) / sizeof(system_dirs[0]));
       i++) {
    vfs_node_t *check = vfs_lookup(system_dirs[i]);
    if (!check) {
      boot_msg("VFS", "System directory not found. Creating...\n", 1);
      if (vfs_mkdir(system_dirs[i], 0755) != 0) {
...
static vfs_node_t *vfs_parent_for_path(const char *path, char *name_buf,
                                       uint32_t buf_size) {
...
int vfs_mkdir(const char *path, uint16_t mode) {
    ...
    if (!parent) {
        printk("VFS_MKDIR: Error, parent not found for %s\n", path);
        return -ENOENT;
    }
...
int vfs_create(const char *path, const char *content) {
  ...
  char name[VFS_NAME_MAX];
  vfs_node_t *parent = vfs_parent_for_path(path, name, sizeof(name));
  if (!parent || parent->type != VFS_TYPE_DIR || !parent->ops ||
      !parent->ops->create)
    return -1;
