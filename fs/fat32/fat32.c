...
void init_fat32() {
    ...
    if (bpb.boot_signature != 0x29) {
        printk("[ FAT32 ] Error: Boot signature not found.\n");
        return;
    }

    printk("[ FAT32 ] Assembled system: %s\n", bpb.volume_label);
    printk("  Clusters of %d sectors\n", bpb.sectors_per_cluster);
...
void fat32_ls(uint32_t cluster) {
    ...
    for (int i = 0; i < 16; i++) {
        ...
        if (entry[i].attributes & 0x0F) {
            for(int j=0; j<8; j++) if(entry[i].name[j] != ' ') printk("%c", entry[i].name[j]);
            if(entry[i].ext[0] != ' ') {
                printk(".");
                for(int j=0; j<3; j++) if(entry[i].ext[j] != ' ') printk("%c", entry[i].ext[j]);
            }

            if (entry[i].attributes & 0x10) {
                printk("    <DIR>");
            } else {
                printk("    <FILE>  %d bytes", entry[i].file_size);
            }
            printk("\n");
        }
    }
