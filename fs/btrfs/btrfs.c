...
int btrfs_read_file(struct vfs_node* node, void* buffer, uint32_t size, uint32_t offset);
int btrfs_write_file_vfs(struct vfs_node* node, const void* buffer, uint32_t size, uint32_t offset);
...
vfs_ops_t bluefs_ops = {
    .create  = btrfs_create_file,
