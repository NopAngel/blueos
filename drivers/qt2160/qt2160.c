...
void qt2160_init() {
  ...
  if (i2c_read_reg(QT2160_I2C_ADDR, QT2160_CHIP_ID, &chip_id) != 0) {
    boot_msg("QT2160", "Failed to communicate via I2C\n", 2);
    return;
  }

  if (chip_id != 0x11) { // 0x11 is the standard ID for QT2160
    boot_msg("QT2160", "Invalid Chip ID\n", 2);
    return;
  }
  ...
  boot_msg("QT2160", "Touch controller initialized successfully.\n", 0);
}
...
void qt2160_poll() {
  ...
  if (i2c_read_reg(QT2160_I2C_ADDR, QT2160_STAT, &status) != 0)
    return;
