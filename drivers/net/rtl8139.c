...
void send_arp_reply(uint8_t *target_mac, uint32_t target_ip) {
  ...
  struct arp_packet *arp =
      (struct arp_packet *)(packet + sizeof(struct eth_header));
...
void send_tcp_ack(uint32_t dest_ip, uint16_t dest_port, uint16_t src_port) {
  ...
  struct tcp_header *tcp =
      (struct tcp_header *)(packet + sizeof(struct eth_header) +
                            sizeof(struct ip_header));

  for (int i = 0; i < 6; i++)
    eth->dest[i] = gateway_mac[i];
  for (int i = 0; i < 6; i++)
    eth->src[i] = mac[i];
...
void send_ping(uint8_t ip1, uint8_t ip2, uint8_t ip3, uint8_t ip4) {
  ...
  for (int i = 0; i < 6; i++)
    eth->dest[i] = gateway_mac[0] == 0 ? 0xFF : gateway_mac[i];
  for (int i = 0; i < 6; i++)
    eth->src[i] = mac[i];
  eth->type = htons(0x0800);
