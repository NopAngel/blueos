...
void i2c_write_byte(unsigned char dev_addr, unsigned char reg_addr,
                    unsigned char data);
...
void adxl345_apply_calibration(struct accel_data *data,
                               struct accel_data *offset) {
  data->x -= offset->x;
  data->y -= offset->y;
  data->z -= offset->z;
}
...
int adxl345_detect_freefall(struct accel_data *data, short threshold) {
  short magnitude = data->x * data->x + data->y * data->y + data->z * data->z;
