...
void pci_write_config(unsigned char bus, unsigned char slot, unsigned char func,
                      unsigned char offset, unsigned int val) {
  unsigned int address =
      (unsigned int)((((unsigned int)bus) << 16) |
...
void register_disk(const char *name_prefix, const char *transport,
                   const char *media_type, uint32_t bar, uint16_t vendor,
                   uint16_t device, uint8_t bus, uint8_t slot, uint8_t func) {
  if (disk_count < 4) {
    ...
    strncpy(system_disks[disk_count].transport, transport,
            sizeof(system_disks[disk_count].transport) - 1);
    strncpy(system_disks[disk_count].media_type, media_type,
            sizeof(system_disks[disk_count].media_type) - 1);
    ...
    if (strcmp(transport, "IDE") == 0) {
      if (disk_is_ssd_primary()) {
        strncpy(system_disks[disk_count].media_type, "SSD",
                sizeof(system_disks[disk_count].media_type) - 1);
      }
      disk_detect_partition_style(
          system_disks[disk_count].partition_style,
...
uint32_t pci_get_vbox_port() { return vbox_io_base; }
...
void detect_piix_pm(uint8_t bus, uint8_t dev, uint8_t func) {
  ...
  if (vendor == 0x8086 && (product == 0x7113 || product == 0x266a)) {
    printk("piixpm0 at pci %d dev %d function %d: vendor 0x%x product 0x%x "
           "(rev. 0x%02x)\n",
    ...
    for (int slot = 0; slot < 32; slot++) {
      ...
      for (int func = 0; func < 8; func++) {
        ...
        if (class_code == 0x0C && subclass == 0x03) {
          ...
          register_disk("u", "USB", "USB", bar0, vendor, device_id, bus, slot,
                        func);
        ...
        if (vendor == 0x1000 && device_id == 0x0012) {
          uint32_t bar0 = pci_read_config(bus, slot, func, 0x10);
          register_disk("s", "SCSI", "HDD", bar0, vendor, device_id, bus, slot,
                        func);
        ...
        if (vendor == 0x8086 && device_id == 0x7010) {
          if (ide_drive_present(0)) {
            ...
            register_disk("h", "IDE", "HDD", bar4, vendor, device_id, bus, slot,
                          func);
            printk(" +   Registered: hd%d (IDE) at %d:%d:%d\n", disk_count - 1,
                   bus, slot, func);
          } else {
            printk(" +   IDE controller found at %d:%d:%d but no primary drive "
                   "attached\n",
                   bus, slot, func);
          }
        }
