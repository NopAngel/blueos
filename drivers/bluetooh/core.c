...
unsigned char bt_send_cmd(unsigned short opcode, unsigned char plen,
                          void *param);
...
void bt_reset(void) { bt_send_cmd(BT_HCI_RESET, 0, 0); }
...
void bt_read_bd_addr(void) { bt_send_cmd(BT_HCI_READ_BD_ADDR, 0, 0); }
...
void l2cap_send_signaling(unsigned short handle, unsigned char code,
                          unsigned char id, unsigned short len, void *data) {
  unsigned char packet[256];
  struct l2cap_hdr *l2hdr = (struct l2cap_hdr *)packet;
  struct l2cap_cmd_hdr *cmdhdr =
      (struct l2cap_cmd_hdr *)(packet + sizeof(struct l2cap_hdr));
  ...
  unsigned char *ptr =
      packet + sizeof(struct l2cap_hdr) + sizeof(struct l2cap_cmd_hdr);
  for (int i = 0; i < len; i++)
    ptr[i] = ((unsigned char *)data)[i];

  unsigned short total =
      sizeof(struct l2cap_hdr) + sizeof(struct l2cap_cmd_hdr) + len;
  bt_send_acl(handle, total, packet);
}

void l2cap_process_signaling(unsigned short handle, unsigned char *data,
                             unsigned short len) {
