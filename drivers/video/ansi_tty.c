...
void ansi_tty_set_color(uint8_t color) { current_color = color; }
  ...
  case 'm':
    for (int i = 0; i <= p_idx; i++) {
      ...
      if (v == 0)
        ...
        current_color |= 0x08; // Bold/Bright
      else if (v >= 30 && v <= 37)
        current_color = (current_color & 0x08) | ansi_to_vga[v - 30];
      else if (v >= 90 && v <= 97)
        current_color = ansi_to_vga[v - 90] | 0x08;
    }
    break;
  case 'A': // Cursor Up
