...
// Puertos de la controladora VGA
...
void vga_set_80x50(void) {
    // 1. Desbloquear registros CRTC
    outb(0x11, VGA_CRT_ICW);
    uint8_t val = inb(VGA_CRT_DATA);
    outb(val & 0x7F, VGA_CRT_DATA);

    // 2. Configurar el Maximum Scan Line (registros internos)
    // Esto le dice a la VGA que use una fuente de 8 pixeles de alto en lugar de 16
    outb(0x09, VGA_CRT_ICW);
    outb(0x07, VGA_CRT_DATA); // 8 pixeles de alto (0-7)

    // 3. Ajustar el cursor para que no desaparezca
    outb(0x0A, VGA_CRT_ICW);
    outb(0x00, VGA_CRT_DATA);
    outb(0x0B, VGA_CRT_ICW);
    outb(0x07, VGA_CRT_DATA);

    // 4. Re-bloquear registros
    outb(0x11, VGA_CRT_ICW);
...
void vga_set_80x25(void) {
    outb(0x09, VGA_CRT_ICW);
    outb(0x0F, VGA_CRT_DATA); // 16 pixeles de alto (0-15)
