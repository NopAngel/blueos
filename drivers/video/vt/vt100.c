...
/**
 * handle_ansi_csi: Parses and executes ANSI Control Sequence Introducer
 * commands.
 */
static void handle_ansi_csi(char command) {
  ...
  case 'm': // Select Graphic Rendition (Colors)
    for (int i = 0; i <= p_idx; i++) {
      int v = params[i];
      if (v == 0)
        color = 0x0F; // Reset to default white
      else if (v == 1)
        color |= 0x08; // Bold/Bright bit
      else if (v >= 30 && v <= 37)
  ...
  case 'B': // Cursor Down
    cursor_y = (cursor_y + (val ? val : 1) < MAX_ROWS)
                   ? cursor_y + (val ? val : 1)
                   : MAX_ROWS - 1;
    break;

  case 'C': // Cursor Forward
    cursor_x = (cursor_x + (val ? val : 1) < MAX_COLS)
                   ? cursor_x + (val ? val : 1)
                   : MAX_COLS - 1;
    break;
  ...
  case 'K': // Erase in Line (0 = from cursor to end)
    for (int i = cursor_x; i < MAX_COLS; i++)
      ((unsigned short *)VIDEO_ADDRESS)[cursor_y * MAX_COLS + i] =
          ' ' | (color << 8);
    break;
  }
}
...
/**
 * vt100_putc: Processes a single character, handling ANSI sequences or raw VGA
 * output.
 */
