...
static inline void gpio_write_reg(unsigned short offset, unsigned int val) {
  ...
  volatile unsigned int *reg =
      (volatile unsigned int *)(GPIO_BASE_ADDR + offset);
  *reg = val;
#endif
}
...
void pinctrl_init() {
  boot_msg("PINCTRL", "Initializing GPIO Controller...\n", 0);

  gpio_write_reg(0x00, 0xFFFFFFFF); // GPIO_USE_SEL
  unsigned int val = gpio_read_reg(0x00);

  if (val != 0) {
    boot_msg("PINCTRL", "GPIO Controller ready\n", 0);
  } else {
    boot_msg("PINCTRL", "Controller not found.\n", 2);
  }
}
