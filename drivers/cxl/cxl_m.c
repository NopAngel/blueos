...
int cxl_enumerate_devices() {
  int found = 0;
  printk("[CXL] Scanning PCI bus for CXL 2.0+ devices...\n");
...
int cxl_init() {
  printk("[CXL] Compute Express Link Subsystem initializing...\n");
  return cxl_enumerate_devices();
}

void cxl_map_memory(struct cxl_device *dev) {
  volatile uint32_t *hdm_base =
      (uint32_t *)(dev->component_regs + CXL_HDM_DECODER0_BASE_LOW);

  uintptr_t cxl_ram_phys = (uintptr_t)(*hdm_base);

  printk("[CXL] Found %d MB of Type 3 Memory at 0x%lx\n",
