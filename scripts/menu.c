...
void init_data() {
    ...
    // GENERAL / DRIVERS / FS 
    strcpy(gen_menu[0].key, "CONFIG_ARCH"); strcpy(gen_menu[0].value, "i386"); strcpy(gen_menu[0].label, "Architecture (i386/x86_64)");
    ...
    strcpy(gen_menu[0].help, "Select the CPU architecture. i386 is 32-bit, x86_64 is 64-bit mode.");
...
int main() {
    ...
    while(1) {
        ...
        if(ch == KEY_UP) sel = (sel > 0) ? sel - 1 : count - 1;
            ...
            if(!strcmp(items[sel].key, "CONFIG_ARCH")) strcpy(items[sel].value, !strcmp(items[sel].value, "i386") ? "x86_64" : "i386");
