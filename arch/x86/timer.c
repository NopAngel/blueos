...
uint32_t timer_get_frequency(void) { return timer_info.frequency_hz; }
...
int timer_set_alarm(uint64_t ms_from_now, void (*callback)(void *),
                    void *data) {
  for (int i = 0; i < MAX_ALARMS; i++) {
    if (!alarms[i].active) {
...
int timer_set_repeating_alarm(uint64_t interval_ms, void (*callback)(void *),
                              void *data) {
  for (int i = 0; i < MAX_ALARMS; i++) {
    if (!alarms[i].active) {
