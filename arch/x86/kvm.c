...
void kvm_unlock_vmx() {
    uint32_t lo, hi;
    __asm__ volatile("rdmsr" : "=a"(lo), "=d"(hi) : "c"(MSR_IA32_FEATURE_CONTROL));

    if (!(lo & 1)) {
        lo |= (1 << 2) | (1 << 0);
        __asm__ volatile("wrmsr" : : "a"(lo), "d"(hi), "c"(MSR_IA32_FEATURE_CONTROL));
...
int kvm_init_vmx(struct vcpu *vcpu) {
    ...
    // __asm__ volatile("vmxon (%0)" : : "r"(&vcpu->vmxon_region_phys) : "cc", "memory");
