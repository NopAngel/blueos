...
int init_amd_svm() {
    ...
    if (ebx != 0x68747541 || edx != 0x69746e65 || ecx != 0x444d4163) { // "Auth" "enti" "cAMD"
        printk(YELLOW, "CPU: No AMD processor detected.\n");
        return 0;
    }
    ...
    if (!(ecx & (1 << 2))) {
        printk(YELLOW, "AMD: This processor does not support SVM technology (AMD-V).\n");
        return 0;
    }
    ...
    printk(GREEN, "AMD: Virtualization (SVM) successfully enabled.\n");
    return 1;
}