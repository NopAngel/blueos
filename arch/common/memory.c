...
void mm_protect_region(uintptr_t base, uint32_t size) {
  ...
  uintptr_t end_bit =
      ((base + size + PAGE_SIZE - 1) - mm.phys_limit_start) / PAGE_SIZE;
...
bool mm_is_kaslr_enabled(void) { return mm.kaslr_enabled; }

uintptr_t mm_get_kernel_slide(void) { return mm.kaslr_slide; }
...
void x86_memory_prepare() { total_memory_mb = 128; }
