...
int init_riscv_vtx_compat() {
    ...
    if (!(misa & (1 << 7))) {
        printk("RISC-V: Hypervisor extension (H) not supported.\n");
        return 0;
    }
    ...
    printk("RISC-V: Hypervisor (H) enabled (Pure C Mode).\n");
    return 1;
}
...
int setup_guest_context() {
    ...
    if (!guest_pt_ptr) {
        printk("RISC-V: Error: Out of memory for Guest Tables.\n");
        return -1;
    }
    ...
    printk("RISC-V: Guest Stage-2 Tables allocated at 0x%lx\n", (uintptr_t)guest_pt_ptr);
    return 0;
}
