...
void profile_init(uintptr_t start_addr, uintptr_t end_addr) {
    ...
    printk("\033[32mPROFILER: Initialized for range 0x%lx - 0x%lx\033[0m\n", start_addr, end_addr);
    printk("\033[90mPROFILER: Buffer allocated at 0x%lx (%d slots)\033[0m\n", (uintptr_t)prof_buffer, prof_len);
}
...
void profile_display() {
    ...
    for (uintptr_t i = 0; i < prof_len; i++) {
        if (prof_buffer[i] > 0) {
            ...
            printk(" Addr 0x%lx: %d hits\n", addr, prof_buffer[i]);
            found_hits++;
        }
    }

    if (found_hits == 0) {
        printk("\033[90m No hits recorded yet.\033[0m\n");
    }
}