...
void blueos_aes_encrypt_buffer(uint8_t *data, uint32_t len, aes_ctx_t *ctx) {
    if (len % 16 != 0) {
        printk(RED, "AES Error: Data length must be multiple of 16\n");
        return;
    }
...
void crypto_init_test() {
    printk(CYAN, "[CRYPTO] Initializing AES-RISCV Hardware Glue...\n");
    ...
    printk(GREEN, "[CRYPTO] Test block encrypted successfully!\n");
}