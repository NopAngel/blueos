...
static inline uint32_t aes32_encrypt_step(uint32_t rs1, uint32_t rs2, int bs) {
  ...
  if (bs == 0)
    asm volatile("aes32esi %0, %1, %2, 0" : "=r"(rt) : "r"(rs1), "r"(rs2));
  else if (bs == 1)
    asm volatile("aes32esi %0, %1, %2, 1" : "=r"(rt) : "r"(rs1), "r"(rs2));
  else if (bs == 2)
    asm volatile("aes32esi %0, %1, %2, 2" : "=r"(rt) : "r"(rs1), "r"(rs2));
  else
    asm volatile("aes32esi %0, %1, %2, 3" : "=r"(rt) : "r"(rs1), "r"(rs2));

  DISABLE_AES_EXT;
  return rt;
}

void aes_encrypt_block_rv32(uint32_t state[4], uint32_t *round_keys,
                            int rounds) {
  // 1. AddRoundKey init
  for (int i = 0; i < 4; i++)
